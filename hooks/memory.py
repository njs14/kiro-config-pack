...
# LTM roll-up (deterministic housekeeping; semantic merging stays manual, see DREAMING.md)
LTM_CONSOLIDATE_THRESHOLD = 30   # full notes before a roll-up is considered
LTM_KEEP_MAX = 50                # hard cap on full notes kept regardless of age
ARCHIVE_AGE_DAYS = 30            # notes older than this become monthly digests
CONSOLIDATE_MIN_INTERVAL_H = 24  # at most one roll-up per day per project
...
# ---------- LTM roll-up: full notes -> monthly digests ----------

def digest_note(path):
    """Title plus up to three 'Asked' bullets; the rest of the note is dropped."""
    title, asked, in_asked = None, [], False
    for ln in open(path, errors="replace").read().splitlines():
        if ln.startswith("# ") and title is None:
            title = ln[2:].strip()
        elif ln.startswith("## "):
            in_asked = ln.strip().lower() == "## asked"
        elif in_asked and ln.startswith("- "):
            asked.append(ln[2:].strip())
    return title or os.path.basename(path)[:-3], asked[:3]

def consolidate_ltm(base):
    """Deterministic housekeeping, no LLM: once the project has more than
    LTM_CONSOLIDATE_THRESHOLD full notes (and at most once per
    CONSOLIDATE_MIN_INTERVAL_H), digest notes older than ARCHIVE_AGE_DAYS —
    plus the oldest beyond LTM_KEEP_MAX — into archive/<YYYY-MM>.md, then
    delete the originals. learned.md, observations.jsonl, and stm.md are
    never touched. Everything written passes scrub()."""
    ltm_dir = os.path.join(base, "ltm")
    if not os.path.isdir(ltm_dir):
        return
    notes = sorted(f for f in os.listdir(ltm_dir) if f.endswith(".md"))
    if len(notes) <= LTM_CONSOLIDATE_THRESHOLD:
        return
    stamp = os.path.join(base, ".last-consolidation")
    now = datetime.datetime.now()
    if os.path.exists(stamp) and (now - datetime.datetime.fromtimestamp(
            os.path.getmtime(stamp))).total_seconds() < CONSOLIDATE_MIN_INTERVAL_H * 3600:
        return

    def note_dt(f):
        try:
            return datetime.datetime.strptime(f[:10], "%Y-%m-%d")
        except ValueError:
            return datetime.datetime.fromtimestamp(os.path.getmtime(os.path.join(ltm_dir, f)))

    cutoff = now - datetime.timedelta(days=ARCHIVE_AGE_DAYS)
    old = [f for f in notes if note_dt(f) < cutoff]
    fresh = [f for f in notes if f not in old]
    if len(fresh) > LTM_KEEP_MAX:
        old += fresh[:len(fresh) - LTM_KEEP_MAX]  # sorted by name = oldest first
    if not old:
        return
    ...
    by_month = {}
    for f in old:
        by_month.setdefault(f"{note_dt(f):%Y-%m}", []).append(f)
    for month, files in by_month.items():
        ap = os.path.join(arch_dir, f"{month}.md")
        existing = open(ap, errors="replace").read() if os.path.exists(ap) else \
        ...
        for f in sorted(files):
            ...
            parts.append(f"## {f[:-3]}")
            parts.append(title)
            parts += [f"- {a}" for a in asked]
            parts.append("")
        with open(ap, "w") as fh:
            fh.write(scrub("\n".join(parts)).rstrip() + "\n")
    for f in old:
        try:
            os.remove(os.path.join(ltm_dir, f))
        except OSError:
            pass
...
def distill(payload):
    ...
    consolidate_ltm(os.path.join(MEM_ROOT, slug))
    return 0
...
def recall(payload):
    ...
    if os.path.isdir(arch_dir):
        months = sorted(f[:-3] for f in os.listdir(arch_dir) if f.endswith(".md"))
        if months:
            out += [f"Older notes are digested by month in {arch_dir}/ ({months[0]} to {months[-1]})."]
    if out:
        print("\n".join(out))
    return 0
...
def main():
    try:
        ...
        if mode == "consolidate":
            consolidate_ltm(os.path.join(MEM_ROOT, project_slug(payload.get("cwd"))))
            return 0
        return 0
    except Exception:
